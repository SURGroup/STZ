 ...
 * \param[in] (cyc_down,cyc_up,hcyc_bottom) V-cycle configuration parameters.
 * \return True if the tolerance was reached before the maximum number of
 * iterations, false otherwise. */
template<class S,class V,class M>
bool tgmg<S,V,M>::solve(int type,int per_loop,int max_loops,double omega,int cyc_down,int cyc_up,int hcyc_bottom) {
    ...
    do {
        ...
        acc=iters_and_error(type,per_loop,omega,cyc_down,cyc_up,hcyc_bottom);
 ...
 * iterations, false otherwise. */
template<class S,class V,class M>
bool tgmg<S,V,M>::solve(int type,tgmg_predict &tp,double omega,int cyc_down,int cyc_up,int hcyc_bottom) {
    int n=tp.lim/tp.mult;

    // Perform the predicted number of smoothing iterations
    double iacc=verbose>=2?l2_error():1,
           acc=iters_and_error(type,n,omega,cyc_down,cyc_up,hcyc_bottom);
    ...
    if(acc<q.acc) {
        ...
        do {
            ...
            n+=k;
            acc=iters_and_error(type,k,omega,cyc_down,cyc_up,hcyc_bottom);
            if(verbose==3) iter_message(n,acc);
        } while(acc>=q.acc);
    }
    ...
    iters(type,tp.extra_iters,omega,cyc_down,cyc_up,hcyc_bottom);
    return !std::isnan(acc);
}
 ...
 * \param[in] hcyc_bottom half the number of Gauss--Seidel sweeps to apply on
 *                        the bottom level of the grid hierarchy.
 * \param[in] symmetric whether to enable a symmetric V-cycle by doing half of
 *                      the Gauss--Seidel sweeps in reverse order. */
template<class S,class V,class M>
void tgmg<S,V,M>::v_cycle(int cyc_down,int cyc_up,int hcyc_bottom,bool symmetric) {
    ...
    if(ml>0) {
        ...
        v_cycle_internal(0,cyc_down,cyc_up,hcyc_bottom,symmetric);
        mg[0]->apply_t();
    }

    // Apply smoothing sweeps on the top level
    up_gs_iterations(cyc_up,symmetric);
}
 ...
 *                      the Gauss--Seidel sweeps in reverse order. */
template<class S,class V,class M>
void tgmg<S,V,M>::v_cycle_internal(int level,int cyc_down,int cyc_up,int hcyc_bottom,bool symmetric) {
    int i;
    ...
    mg[ml-1]->down_gs_iterations(hcyc_bottom);
    mg[ml-1]->up_gs_iterations(hcyc_bottom,symmetric);

    // Propagate the solution up the hierarchy, smoothing at each step
    for(i=ml-1;i>level;i--) {
        mg[i]->apply_t();
        mg[i-1]->up_gs_iterations(cyc_up,symmetric);
    }
}
...
template<class S,class V,class M>
void tgmg<S,V,M>::fmg_pre(int cyc_down,int cyc_up,int hcyc_bottom,bool symmetric) {
    int i,j;
    ...
    if(ml>0) {
        ...
        for(i=ml-1;i>0;i--) {
            mg[i]->apply_t();
            mg[i-1]->up_gs_iterations(cyc_up,symmetric);
            for(j=0;j<cyc_down;j++) mg[i-1]->gauss_seidel();
            v_cycle_internal(i-1,cyc_down,cyc_up,hcyc_bottom,symmetric);
        }
    ...
    // Apply a final set of Gauss--Seidel sweeps
    up_gs_iterations(cyc_up,symmetric);
}
