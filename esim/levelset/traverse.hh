...
void traverse<T,E>::extrapolate_staggered(int i,int ij) {
	...
	// Test for blocks that have been completed by filling in this grid
	// point
	...
	// This list will usually have one or two elements so a bubble sort
...
void levelset::extrapolate_fields(E& e) {
	...
	// Check that the mask counter has enough space for mn separate
	// entries, otherwise reset it.
	if(bpc+1<1) reset_back_pointers();

	// Mark all the points on the interior of the band from which values
