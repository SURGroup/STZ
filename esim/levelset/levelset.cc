...
levelset::levelset(int m_,int n_,double ax_,double ay_,double dx_,double dy_,double phi_down_,double phi_up_) :
	m(m_), n(n_), mn(m*n), me(m+1), ne(n+1), mne(me*ne),
	ax(ax_), ay(ay_), bx(ax+m*dx_),
	by(ay+n*dy_), dx(dx_), dy(dy_), xsp(1/dx), ysp(1/dy),
	dx2(dx*dx), dy2(dy*dy), phi_down(phi_down_), phi_up(phi_up_),
	u_mem(u_init_memory), bpc(mn+1), sgc(1), mem_t1(temp_init_memory),
	mem_p1(temp_init_memory),
	mem_up(heap_init_memory),mem_down(heap_init_memory),
	s(new int[mn]), ss(new int[mne]),
	bp(new unsigned int[mn]), sg(new unsigned int[mne]),
	phi(new double[mn]),vp(new double[mn]),
	u(new int[u_mem]),
	h_up(new int[mem_up]),h_down(new int[mem_down]),
	t1(new int[mem_t1]), p1(new double[mem_p1]), sort(phi),
	failsafe(false) {
	unsigned int *p=bp;while(p<bp+mn) *(p++)=0;
	p=sg;while(p<sg+mne) *(p++)=0;
}
...
void levelset::add_memory_t1() {
	...
	fprintf(stderr,"Add temporary integer memory\n");
#endif
	mem_t1<<=1;
...
/** Carries out a bicubic interpolation of the level set function at
 * a point, also calculating the level set gradient at that point.
...
/** Carries out the modified Newton--Raphson iteration of Chopp, to find a point
 * where the bicubic interpolation of the level set function is zero, which is
 * closest to a given grid point.
...
/** In cases where the Newton iteration routine fails, this routine provides
 * a lower-order backup following the first order interpolation procedure
...
void levelset::build_band() {
	...
	// Store initial counts of the number of points either side of the interface
	num_n=u_start;
	num_p=u_end;

	// Compute new values for points either side of the interface by calling the
	// Newton iteration. Store the values in the temporary floating point array
	for(k=0;k<u_end;k++) {
		ij=u[k];i=ij%m;
	...
	// Se(t the counters of the number and negative and postive boundary points.
	// num_n is initially set to the memory reference of the last negative point.
	// This value is used when the fast marched points are meshed with the
	// boundary points.
	num_p=u_end;
	num_n=u_start;
