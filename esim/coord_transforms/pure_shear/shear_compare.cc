...
int main(int argc,char **argv) {
    ...
    const double tf = 1./lamb;
    const double u_bdry = 0;
	...
	shear_sim dsim(4*ss[run], ss[run], -4, 4, -1, 1, mu, K, visc, t_scale, tmult, adapt_fac, u_bdry, lamb, &stz, "");
	shear_sim qsim(4*ss[run], ss[run], -4, 4, -1, 1, mu, K, visc, t_scale, tmult, adapt_fac, u_bdry, lamb, &stz, "");
