 ...
 */
            ...
            /*
             *Old, less readable way - keep it here because it's definitely correct
             */

            (uc > 0)? rmv_eno2(ux, vx, Xx, Yx, hx, fr, f, fl, fp[-2],
                      (i == m-1)? lx : 0, (i == 0)? -lx : 0, (i <= 1)? -lx : 0)
                : rmv_eno2(ux, vx, Xx, Yx, -hx, fl, f, fr, fp[2],
                      (i == 0)? -lx : 0, (i == m-1)? lx : 0, (i >= m-2)? lx : 0);

            (vc > 0)? ((j > 1)? rmv_eno2(uy, vy, Xy, Yy, hy, fu, f, fd, fp[-2*gm])
                 : rmv_one_sided(uy, vy, Xy, Yy, hy, f, fd))
                : ((j < n-1)? rmv_eno2(uy, vy, Xy, Yy, -hy, fd, f, fu, fp[2*gm])
                   : rmv_one_sided(uy,vy,Xy,Yy,-hy,f,fu));

			// Calculate net force due to stress imbalance
            // Fills fx = -dp_dx - dq_dx + ds_dx + dt_dy
            //       fy = -dp_dy - dq_dy - ds_dy + dt_dx
            ...
            // Recall that in rescaled coordinates mu = rho
 ...
 */
         /*
          *Older, less readable way - keep here because it's definitely correct
          */
...
#pragma omp parallel for
	for(int j = 0; j < n; j++) for(int i = 0; i < m; i++) {
        ...
        // uc will hold u at the central location
        // vc will hold v at the central location
        // two_omega is twice the spin value
        // ddev will hold the result of the adaptive_plastic_term algorithm - 2*dt*mu*Dpl/sbar
		...
		if (j > 0) {
            ...
            if (uc > 0) rm_eno2(Xx, Yx,  hx, fr, f, fl,  fp[(i <= 1)? m-2 : -2], (i == m-1)? lx : 0,    (i==0)? -lx : 0,  (i <= 1)? -lx : 0);
            else        rm_eno2(Xx, Yx, -hx, fl, f, fr, fp[(i >= m-2)? 2-m : 2],  (i == 0)? -lx : 0, (i == m-1)? lx : 0, (i >= m-2)? lx : 0);
            
            if (vc > 0){
                if (j > 1) rm_eno2(Xy, Yy, hy, fu, f, fd, fp[-2*m]);
                else       rm_one_sided(Xy, Yy, hy, f, fd);
            }
            else{
                if (j < n-1) rm_eno2(Xy, Yy, -hy, fd, f, fu, fp[2*m]);
                else rm_one_sided(Xy, Yy, -hy, f, fu);
            }


/*          Old, less readable way - keep it commented because it's definitely correct
 *
 ...
 */
		...
		two_omega = hx*(-f.v + fr.v - fp[m].v + frp[m].v) - hy*(-f.u - fr.u + fp[m].u + frp[m].u);

		// Calculate ENO derivatives of staggered fields
		f1 = (uc>0)? st_eno2(hx, fr, f, fl, fp[(i <= 1)? m-2 : -2]) : st_eno2(-hx, fl, f, fr, fp[(i>=m-2)? 2-m : 2]);

        if (vc > 0){
            if (j > 0)   f2 = st_eno2(hy, fu, f, fd, fp[-2*m]);
            else         f2 = st_one_sided(hy, f, fd);
        }
        else {
            if (j < n-1) f2 = st_eno2(-hy, fd, f, fu, fp[2*m]);
            else         f2 = st_one_sided(-hy, f, fu);
        }
