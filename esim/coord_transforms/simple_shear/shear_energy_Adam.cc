...
int main(int argc,char **argv) {
    ...
    const double mu = mu_phys/s_y;
    const double K = 2*mu*(1+nu)/(3*(1-2*nu));
    ...
    const double le=4.01e-8;                   // Length scale (m)
    ...
    const double u_bdry_phys=0.5*s_rate*le; // Wall speed (m/s)
    ...
    const double lamb_phys=1e8;                  // Strain rate (1/s)
    const double lamb=1e8*t_scale;               // Strain rate in simulation units
    ...
    shear_sim sim(32,32,0,1,0,1,mu,K,
    ...
    qs?sim.solve_quasistatic(drn,n_frames,steps):sim.solve(drn,160);
}
