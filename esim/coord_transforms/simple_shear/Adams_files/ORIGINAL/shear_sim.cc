...
shear_sim::shear_sim(const int m_,const int n_,const double ax_,const double bx_,
	...
	chi_len(chi_len_), t_scale(t_scale_), tmult(tmult_), adapt_fac(adapt_fac_),
	u_bdry(u_bdry_), bfx(bfx_), bcs(4), stz(stz_), filename(filename_),
	fm((new c_field[mn+2*m])+m), vel(new vec[mn+m]), src(new vec[mn+m]),
	tr(NULL), time(0.0), f_num(0), qsm(*this), mg(qsm,src,vel),
...
void shear_sim::write_files(int k) {
	const int fflags=64|128|256|4096|8192;
