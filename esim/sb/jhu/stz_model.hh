...
class stz_rheology : public stz_dynamics {
        public:
		const double c_eff;
		const double A;
		const double n;
		const double n_inv;
                stz_rheology(double TZ_,double chi_inf_,double c_eff_,double A_,double n_) :
                        stz_dynamics(TZ_,chi_inf_), c_eff(c_eff_), A(A_), n(n_), n_inv(1./n) {};
                ~stz_rheology() {};
                virtual double Dplastic(double sbar,double chi,double &dchi1,double &dchi2) {
			...
			// rheological law
			double Dpl=pow(((sbar-1)/A),n_inv)*exp(1./chi_inf-1./chi);

			// Compute changes to chi
                        dchi2=0;
			dchi1=2*Dpl*sbar/c_eff;
                        return Dpl;
                }
};
