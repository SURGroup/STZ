...
class shear_sim {
	public:
		...
		const int mn;
		/** The flags governing what fields to output. */
		...
		const double u_bdry;
		/** The horizontal body foce per unit volume. */
		const double bfx;
		...
		double time;
		/** The current horizontal body force. */
		double bfxt;
		...
		shear_sim(const int m_,const int n_,const double ax_,const double bx_,
			  ...
			  const double bfx_,stz_dynamics *stz_,const unsigned int fflags_,const char *filename_);
		~shear_sim();
		void calc_def_rate();
