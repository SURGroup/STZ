...
qs_multi::qs_multi(shear_sim &ss) : m(ss.m), n(ss.n+1), mn(m*n), x_prd(true),
	y_prd(false), z(ss.vel), K(ss.K+ss.mu/3.0), qK(.25*K*ss.xsp*ss.ysp),
	Kmu(K+ss.mu), mu(ss.mu), dx(ss.dx), dy(ss.dy), xsp(ss.xsp),
	ysp(ss.ysp), xsp2(ss.xsp*ss.xsp), ysp2(ss.ysp*ss.ysp),
	xyfac(xsp2+ysp2), tmufac1(Kmu*xsp2+mu*ysp2), tmufac2(mu*xsp2+Kmu*ysp2),
	bcs(ss.bcs), viscdt_prev(-1), regz(mat(0)),
	acc(tgmg_accuracy(2*(tmufac1+tmufac2),1e4)), mg(*this,ss.src,z), tp() {}
