...
while(1) {
	...
	# Create stepped fields if requested
	if($opt_s) {
		system "../../shared/gpm_process -s $infile temp_mat$P";
		$infile="temp_mat$P";
		if($opt_c) {
			system "../../shared/gpm_process -s $infile2 temp_mat${P}d";
			$infile2="temp_mat${P}d";
		}
	}
	...
	exec $ex."gnuplot temp$P.gnuplot 2>/dev/null" if ($pid[$P]=fork)==0;
...
unless ($opt_f) {
	$mn="${e}_".($opt_c?($ARGV[1] eq $ARGV[3]?"${e2}_$ARGV[1]":"$ARGV[1]_${e2}_$ARGV[3]"):$ARGV[1]).".mov";
	unlink $mn;
	system "qt_export --sequencerate=50 $o/fr_0001.png --loadsettings=../../misc/qtprefs/qt --replacefile $mn";
	#system "rm -rf $o";
}
