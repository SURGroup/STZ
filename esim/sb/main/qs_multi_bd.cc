...
qs_multi_bd::qs_multi_bd(bd_sim &bd) : m(bd.m+1), ml(bd.m), n(bd.n+1), mn(m*n),
	x_prd(false), y_prd(false), fm(bd.fm), z(bd.vel), b(bd.src), eforce(bd.eforce),
	phi(bd.phi), c(bd.c), cc(bd.cc), acc(1e-20), K(bd.K+bd.mu/3.0),
	qK(.25*K*bd.xsp*bd.ysp), Kmu(K+bd.mu), mu(bd.mu), dx(bd.dx), dy(bd.dy),
	xsp(bd.xsp), ysp(bd.ysp), xsp2(bd.xsp*bd.xsp), ysp2(bd.ysp*bd.ysp),
	xyfac(xsp2+ysp2), tmufac1(Kmu*xsp2+mu*ysp2), tmufac2(mu*xsp2+Kmu*ysp2),
	bcs(1), ocs(1e-3), ncs(0.14), rz(mat(0)), rc(mat(0,qK,qK,0)), mrc(mat(0,-qK,-qK,0)),
...
#pragma omp parallel for
	for(int j=0;j<n;j++) for(int i=0,ij=j*m;i<m;i++,ij++) {
		...
		if(*ccp<4) {
			d[ij]=-1;
			c_field *fp=fm+ij,&f0=fp[-m-1],&f1=fp[-m],&f2=fp[-1],&f3=*fp;
		...
		if(*cp<4||cp[-1]<4||cp[-ml]<4||cp[-m]<4||ccp[-m-1]<4||ccp[-m]<4
		  ||ccp[-ml]<4||ccp[-1]<4||ccp[1]<4||ccp[ml]<4||ccp[m]<4||ccp[m+1]<4) {

			// Compute the normal vector
			double *phip=phi+ijl,
				nx=xsp*(*phip-phip[-1]+phip[-ml]-phip[-m]),
				ny=ysp*(*phip+phip[-1]-phip[-ml]-phip[-m]),
				nsq=nx*nx+ny*ny;
			if(nsq>1e-50) {
				nsq=1/sqrt(nsq);
				nx*=nsq;ny*=nsq;
