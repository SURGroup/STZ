...
bd_sim::bd_sim(const int m_,const int n_,const double ax_,const double bx_,
	...
	chi_len(chi_len_), filter_stress(filter_stress_), t_scale(t_scale_), tmult(tmult_),
	adapt_fac(adapt_fac_), stz(stz_), filename(filename_), fflags(fflags_),
	ls(m,n,ax+0.5*dx,ay+0.5*dy,dx,dy,-5*dx,5*dx), phi(ls.phi), c(ls.s),
	cc(ls.ss), fm((new c_field[mne])), vel(new vec[mne]),
	src(new vec[mne]), time(0), bc_smooth(0), f_num(0), wrong_spin(false),
	apply_chi_diffusion(false), eforce(NULL),
	qsm(*this), mg(qsm,src,vel), buf(new float[m>=63?m+2:64]), e_rm(fm),
	e_st(fm), e_reg(fm,m) {
...
void bd_sim::init_bar(int ca) {
	int j;
//	const double pi=3.1415926535897932384626433832795;
	const double Bx=-1,Bw=0.65,Bc=0.3;
	const double Bspc=0.4;

	// Set up the level set to describe the object boundary
	double *phip=phi;
	for(j=0;j<n;j++) {
		double x,y=ay+(j+0.5)*dy;
		if(ca==1) {
			for(int i=0;i<m;i++) {
				x=ax+(i+0.5)*dx-Bx;
				*(phip++)=x>-Bc?(-fabs(y)>x?-x-Bw:fabs(y)-Bw)
					:(fabs(y)<Bc?-x-Bw:Bc-Bw+sqrt((x+Bc)*(x+Bc)+(fabs(y)-Bc)*(fabs(y)-Bc)));
			}
		} else if(ca==2) {
			for(int i=0;i<m;i++) {
				x=ax+(i+0.5)*dx;
				double sp,tphi=fabs(x)-0.25;
				sp=0.15-sqrt(x*x+(y-Bspc)*(y-Bspc));if(sp>tphi) tphi=sp;
				sp=0.15-sqrt(x*x+(y-2*Bspc)*(y-2*Bspc));if(sp>tphi) tphi=sp;
				sp=0.15-sqrt(x*x+(y+Bspc)*(y+Bspc));if(sp>tphi) tphi=sp;
				sp=0.15-sqrt(x*x+(y+2*Bspc)*(y+2*Bspc));if(sp>tphi) tphi=sp;
				*(phip++)=tphi;
			}
...
void bd_sim::direct_step(double dt) {
	...
	// Move the level set
	ls.move_and_smooth(this,dt,bc_smooth);
	set_staggered_status();
...
#pragma omp parallel for
	for(int j=0;j<n;j++) {
		...
		for(int i=0,*cp=c+j*m,*ccp=cc+j*me;i<m;i++,cp++,ccp++) {
			...
			if(*cp<4) {
				...
				// Debug: wrong spin
				if(wrong_spin) two_omega=-two_omega;
	...
	// Add in the chi diffusion if needed
	if(chi_len>0) chi_diffusion(dt);
	...
	for(int j=0;j<n;j++) {
		...
		for(int i=0,*cp=c+j*m,*ccp=cc+j*me;i<m;i++,cp++,ccp++) {
			...
			if(j>0&&i>0&&*ccp<4) {
				...
				// Calculate ENO derivatives of regular fields
				uc>0?(i>1&&ccp[-2]<4?rm_eno2(Xx,Yx,hx,fr,f,fl,fp[-2])
						    :rm_one_sided(Xx,Yx,hx,f,fl))
				    :(i<m-1&&ccp[2]<4?rm_eno2(Xx,Yx,-hx,fl,f,fr,fp[2])
...
#pragma omp parallel for
	for(int j=0;j<n;j++) for(int i=0;i<m;i++) {
		...
		if(c[ij]<4) {
			double ux=hx*(-vp->x+vp[1].x-vp[me].x+vp[me+1].x),
			       uy=hy*(-vp->x-vp[1].x+vp[me].x+vp[me+1].x),
			       vx=hx*(-vp->y+vp[1].y-vp[me].y+vp[me+1].y),
			       vy=hy*(-vp->y-vp[1].y+vp[me].y+vp[me+1].y);
			f.p+=-K*(ux+vy);
			f.q+=-mu*third*(ux+vy);
			f.s+=mu*(ux-vy);
			f.tau+=mu*(uy+vx);
...
void bd_sim::set_boundaries() {
	...
	for(int j=1,ij=me;j<n;j++,ij++) {
		if(cc[ij]<8) {
			fm[ij].u=0;
			fm[ij].v=-0.000;
			fm[ij].X=ax;fm[ij].Y=ay+j*dy;
		}
		ij+=m;
		if(cc[ij]<8) {
			fm[ij].u=0;
			fm[ij].v=-0.000;
			fm[ij].X=bx;fm[ij].Y=ay+j*dy;
		}
	}
...
/** Calculates the extremal values of pressure, deviatoric stress, effective
 * temperature and plastic deformation rate.
 * \param[in] gex a pointer to 16-element array in which to store the values.
 *		  The first eight are minima and maxima of the smoothed fields
 *		  (to remove any checkerboarding), and the next eight are minima
 *		  and maxima of the original fields. */
void bd_sim::extrema(double *gex) {

	double dt=dx*dx*tmult,itdtmu=1/(2*dt*mu),dpls,ddevm;
	extrema_init(gex);

	// Find any gridpoint within the body
#pragma omp parallel for
	for(int j=0;j<m;j++) {
		double lex[16],*lp,de,ps,des,chis;
		c_field *fp=fm+me*j;
		int *cp=c+m*j;
		extrema_init(lex);
		for(int i=0;i<m;i++) if(cp[i]<4) {
			...
			if(i>0&&cp[i-1]<4&&i<m-1&&cp[i+1]<4&&j>0&&cp[i-m]<4&&j<m-1&&cp[i+m]<4) {
				...
				ddevm=fpp[-me].ddev+fpp[-1].ddev+fpp[1].ddev+fpp[me].ddev+4*fpp->ddev;
				dpls=ddevm*itdtmu;
				lp=lex;
				extrema_check(0.125*ps,lp);
				extrema_check(0.125*des,lp);
				extrema_check(0.125*chis,lp);
				extrema_check(0.125*dpls,lp);
			} else lp=lex+8;
			...
			extrema_check(fpp->ddev*itdtmu,lp);
		}

#pragma omp critical
		{
			double *p=lex,*pe=p+16,*pg=gex;
			while(p<pe) {
				if(*p<*pg) *pg=*p;
				p++;pg++;
				if(*p>*pg) *pg=*p;
	...
	gex[10]=sqrt(gex[10]);gex[11]=sqrt(gex[11]);
	gex[4]*=TZ;gex[5]*=TZ;
	gex[12]*=TZ;gex[13]*=TZ;
}

double bd_sim::curvature(double x,double y) {
	double fx=(x-ax)*xsp,fy=(y-ay)*ysp;
	int i=static_cast<int>(fx),j=static_cast<int>(fy);
	if(i<0) i=0;else if(i>m-2) i=m-2;
	if(j<0) j=0;else if(j>n-2) j=n-2;
	fx-=i;fy-=j;

	double *phip=phi+i+m*j;
	return (1-fy)*((1-fx)*curvature(i,j,phip)+fx*curvature(i+1,j,phip+1))
	      +fy*((1-fx)*curvature(i,j+1,phip+m)+fx*curvature(i+1,j+1,phip+m+1));
}

/** Computes the curvature of the level set function at given gridpoint,
 * multiplied a smoothed delta function.
 * \param[out] (phix,phiy) the gradient of the level set function.
 * \param[out] bfdelta the curvature multiplied by a smoothed delta function.
 * \param[in] (i,j) the gridpoint coordinates.
 * \param[in] ij the gridpoint index. */
double bd_sim::curvature(int i,int j,double *phip) {
	const double bd_epsilon=std::numeric_limits<double>::epsilon();
	const double bd_tol=100*bd_epsilon*bd_epsilon;
	double phih=*phip;
	double phil=i==0?2*phih-phip[+1]:phip[-1],phir=i==m-1?2*phih-phip[-1]:phip[+1],
	       phid=j==0?2*phih-phip[+m]:phip[-m],phiu=j==n-1?2*phih-phip[-m]:phip[+m],

	// Compute the gradient of the level set field
	       phix=0.5*xsp*(phir-phil),phiy=0.5*ysp*(phiu-phid);

	// If modulus of the level set value is smaller than the smoothing
	// distance, then compute the curvature
	double modphi=sqrt(phix*phix+phiy*phiy);
	if(modphi>bd_tol) {
		modphi=1/sqrt(modphi);
		double phidl=j==0?(i==0?2*phih-phip[+m+1]:2*phil-phip[+m-1]):(i==0?2*phid-phip[-m+1]:phip[-m-1]),
		       phidr=j==0?(i==m-1?2*phih-phip[+m-1]:2*phir-phip[+m+1]):(i==m-1?2*phid-phip[-m-1]:phip[-m+1]),
		       phiul=j==n-1?(i==0?2*phih-phip[-m+1]:2*phil-phip[-m-1]):(i==0?2*phiu-phip[+m+1]:phip[+m-1]),
		       phiur=j==n-1?(i==m-1?2*phih-phip[-m-1]:2*phir-phip[-m+1]):(i==m-1?2*phiu-phip[+m-1]:phip[+m+1]),
		       phixx=xsp*xsp*(phir-2*phih+phil),
		       phixy=0.25*xsp*ysp*(phiur-phiul-phidr+phidl),
		       phiyy=ysp*ysp*(phiu-2*phih+phid);
		return (phixx*phiy*phiy+phiyy*phix*phix-2*phixy*phix*phiy)*(modphi*modphi*modphi);
	}
	return 0;
}
