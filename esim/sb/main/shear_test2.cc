...
int main(int argc,char **argv) {
	...
	shear_sim sim(320,320,-2,2,-2,2,mu,K,visc,0,0,t_scale,tmult,adapt_fac,u_bdry,&stz,qs?qfn:dfn);
	sim.init_fields(0);

	// Code to read in a text file
	FILE *fp=fopen("test_chi_data.txt","r");
	if(fp==NULL) {
	...
	int i,j,mm,nn;
	if(fscanf(fp,"%d %d",&mm,&nn)!=2) fputs("File input error",stderr);
	double *arr=new double[mm*nn],*arp=arr;

	// Read each value into the array
	for(j=0;j<nn;j++) for(i=0;i<mm;i++,arp++)
		if(fscanf(fp,"%lf",arp)!=1) fputs("File input error",stderr);
	fclose(fp);

	// Initialize the chi field as the bicubic interpolation of the array
	sim.initialize_chi_bicubic(mm,nn,arr,0.025,0.008);
	delete [] arr;

	// Carry out the simulation using the selected simulation method
	qs?sim.solve_quasistatic(200,200,10):sim.solve(200,200);
}
