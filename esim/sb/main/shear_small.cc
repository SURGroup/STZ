...
int main(int argc,char **argv) {
	...
	const double adapt_fac=10;		// Adaptivity factor
	...
	shear_sim sim(128,64,-2,2,-1,1,mu,K,visc,0,0,t_scale,tmult,adapt_fac,u_bdry,&stz,qs?qfn:dfn);
	sim.init_fields(0,600,200);

	// Carry out the simulation using the selected simulation method
	qs?sim.solve_quasistatic(2e6/sca,200,10):sim.solve(2e6/sca,200);
}
