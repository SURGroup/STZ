...
while(-e "${e}/$ARGV[2].$a") {
	...
	if($a<43) {
		...
		$wtt-=4.246554927746;
		$wtt*=1e9;
		$tscaled="t_V+".($wtt<1e-4?($wtt==0?"0\\textsf{~s}":sprintf "%.2g\\textsf{~fs}",$wtt*1e6):
			($wtt<0.1?sprintf "%.2g\\textsf{~ps}",$wtt*1e3:
				  sprintf "%.2g\\textsf{~ns}",$wtt));
		$tscaled="t_V" if $a==43;
		$type="(Direct)";
	}
	$wps=sprintf "%.4g",$wp;
...
#	}
	...
	exec "gnuplot temp$P.gnuplot; ./add_key.pl temp_out$P.tex 3; epstopdf temp_out${P}-inc.eps; pdflatex -interaction=batchmode temp_out$P.tex; convert -density 600 -background white -flatten +matte -filter lanczos -geometry 650x650 temp_out$P.pdf png24:$of.png" if (($pid[$P]=fork)==0);
