...
def ego(optimizationModel, reference, initParams, checkPrevious=True, 
    ...
    if checkPrevious and os.path.isfile(prevFileName):
        ...
        if om.samplingstyle == 0:
            ...
            *aa, = np.meshgrid(*AA.T)
            ...
            x1 = LHS(dist_object=A, nsamples=om.nInitialObs, random_state=np.random.RandomState(780), verbose=False)
            X = x1.samples
    ...
    while len(Y) < optimizationModel.nTotalObs and Y_best > optimizationModel.dtol:
        ...
        if iObs < optimizationModel.nInitialObs:
            ...
            # Generate surrogate points by Sobol sequencing
            A = []
            for param in initParams.parameters:
                if not param.is_constant:
                    A.append(Uniform(loc=param.vmin, scale=param.vmax-param.vmin))
            x1 = LHS(dist_object=A, nsamples=om.nSurrEvals, random_state=np.random.RandomState(780), verbose=False)
            X_surr = x1.samples
...
def main():
    ...
    om = OptimizationModel(nInitialObs=650, nTotalObs=2500, nSurrEvals=1500000, \
                            samplingstyle=3,\
